...
| **`$01000 - $0BC41`** | 44,098 B | **PCM Track 1**: `AUDIO_GAME_START` (6.80s authentic arcade opening theme, fades smoothly to exact zero). |
| **`$0BC42 - $0CB58`** | 3,863 B | **PCM Track 2**: `AUDIO_BOMB` (0.60s falling bomb whistle). |
| **`$0CB59 - $0DD7B`** | 4,643 B | **PCM Track 3**: `AUDIO_PICKUP` (0.72s full 9-burst rescue melody, all pitch variations). |
| **`$0DD7C - $0FBE2`** | 7,782 B | **PCM Track 4**: `AUDIO_BIG_EXPLOSION` (1.20s heavy arcade explosion, fades smoothly to exact zero). |
| **`$0FBE3 - $0D7EF`** | **5,902 B** | **Bank 0 PCM Safe Margin**. |
| **`$0D7F0 - $0EFFF`** | **6,160 B** | **High Score BGM PSG Table** (`AUDIO_HIGHSCORE`): 593 event records × 4 bytes = 2,372 B active, followed by end-marker. Streams directly from VRAM to PSG registers at 60Hz; zero host RAM usage. |
| **`$0F000 - $0FFFF`** | **4,096 B** | **Bank 0 Safe Free Margin** (avoids hardware register boundary). |
...
- **100% Zero-Disk Runtime Audio**: All 15 arcade PCM samples are loaded into VRAM once at boot via MLI direct block reading (`build/pcm.blob` at `PCM_START_BLOCK` = 200, 87,837 bytes, 172 blocks). Zero disk I/O occurs during gameplay!
  - **Bank 0 Resident (`$1000..$FBE2`, 60,386 B)**: `AUDIO_GAME_START` (6.80s opening theme) + `AUDIO_BOMB` (0.60s falling bomb whistle) + `AUDIO_PICKUP` (0.72s complete 9-burst rescue melody) + `AUDIO_BIG_EXPLOSION` (1.20s heavy arcade explosion). **1,054 bytes safe buffer** below `$FFFF`.
...
| :--- | :--- | :--- | :--- |
| **Channels 0–3**<br>(4-Voice Theme) | **`AUDIO_GAME_START`** | Stage 1 announce banner appears | **Cycle-Accurate 4-Voice PSG Chiptune** (`src/theme_psg.h`): 435-frame event table streamed from SRAM at 60Hz; 100% replaces 44KB PCM; saves 89 disk blocks. |
| **Channels 0–3**<br>(4-Voice Loop) | **`AUDIO_HIGHSCORE`** | High Score initials entry screen | **VRAM-Resident PSG BGM** (`HIGHSCORE_PSG_VRAM_ADDR=$D7F0`, Bank 0): 593 events × 4B streamed directly from VRAM to PSG at 60Hz, SPEEDUP=1.06 to match CX16 PCM tempo; loops until initials confirmed. |
...
| **Channel 2 & 5**<br>(Dual-Voice Burst) | **`AUDIO_ENEMY_EXPLODE`** | Enemy plane shot down | **High/Low Dual-Wave Burst**:<br>• Ch 2: White noise fast descending sweep<br>• Ch 5: Sawtooth low bass rumble (50Hz sub)<br>• Smooth decay to 0 volume; zero cutoff click on rapid kills! |
...
| :---: | :--- | :--- | :---: | :---: | :---: | :--- |
| **Bank 0** | **`AUDIO_GAME_START`** | `game_start.pcm` | 44,098 B | 6.80 s | **Vol 11/15** | 1982 arcade opening theme (fades cleanly to exact zero). |
| **Bank 0** | **`AUDIO_BOMB`** | `bomb.pcm` | 3,863 B | 0.60 s | **Vol 10/15** | 1940 heavy bomber falling bomb whistle. |
| **Bank 0** | **`AUDIO_PICKUP`** | `pickup.pcm` | 4,643 B | 0.72 s | **Vol 8/15** | Authentic 1982 arcade parachute rescue (full 9-burst multi-tone melody). |
| **Bank 0** | **`AUDIO_BIG_EXPLOSION`** | `big_explosion.pcm` | 7,782 B | 1.20 s | **Vol 13/15** | Heavy arcade explosion for player crash, boss death, and bomber (fades cleanly to exact zero). |
...
| **Bank 1** | **`AUDIO_WAVE_START`** | `wave_start.pcm` | 2,217 B | 0.34 s | **Vol 8/15** | 4-plane formation attack alert siren (balanced level). |
...
68. **Boot Sequence Title Screen Pre-Render & Coin Drop Audio Stutter Elimination**:
...
69. **VERA Bank 0 PCM Audio Expansion & Explosion Tail Pop Noise Elimination (`AUDIO_PICKUP` & `AUDIO_BIG_EXPLOSION`)**:
    - **Bank 0 Multi-Sample Fitting**: Re-balanced Bank 0 sample durations (`AUDIO_GAME_START` 6.80s, `AUDIO_BIG_EXPLOSION` 1.20s) to free up headroom, accommodating the **full 0.72-second multi-tone rescue melody** (`pickup.pcm`, 4,643 bytes at 6,485 Hz). Bank 0 holds 60,386 bytes with **1,054 bytes safe margin** below `$FFFF`.
    - **Full 9-Burst Melodic Progression & Balanced Volume**: Captured the complete 9-burst frequency sweep sequence (1270Hz ➔ 840Hz ➔ 1050Hz ➔ 850Hz ➔ 1180Hz ➔ 710Hz ➔ 1260Hz ➔ 675Hz) with high and low tones, distinct from the monotone 4-squadron alert. Tuned playback volume from 12 down to 8 for a pleasant, balanced arcade reward sound that doesn't overpower the dogfight.
    - **Elimination of Explosion Tail Pop Noise ("bh" Click)**: Identified root cause of the tail pop noise in `mkpcm_blob.mjs`: `res.fill(0x80)` erroneously filled the tail with `0x80` (-128 maximum negative voltage), stepping DC offset abruptly from 0 to -128 before FIFO drain. Replaced with `0x00` (true signed silence), refined window fade to reach exact 0.0, and updated PSG `CH_EXPL` to decay volume to 0 before silence, completely eliminating all tail pops and clicks across both PCM and PSG explosions.
    - **Zero Runtime Disk I/O & Total 15 Resident PCM Samples**: Total resident audio blob holds 15 authentic arcade samples (87,837 bytes, 172 ProDOS blocks), all loaded once at boot with zero disk I/O during gameplay.
70. **Dual 140KB 5.25" Floppy Disk Architecture (`TimePilot-IIvera-D1.po` + `TimePilot-IIvera-D2.po`)**:
    - **Dual-Floppy Geometry & Packaging**: Successfully packaged the complete Time Pilot IIvera game (with all 15 PCM samples, 33 art arrays, and both Slot 2 and Slot 4 binaries) into two standard Apple II 140KB 5.25" floppy disks (280 blocks each), leaving generous free space on both disks:
      - **Disk 1 (`TimePilot-IIvera-D1.po`, Volume `TIMEPILOT1`)**: Retains `PRODOS` (34 blocks) and `BASIC.SYSTEM` (21 blocks), and holds `STARTUP` (4 blocks), `MAIN.BIN` (Slot 2 binary, 61 blocks), and `ART` (118 blocks with data at fixed block 128). 245/280 blocks used (**35 blocks free**).
      - **Disk 2 (`TimePilot-IIvera-D2.po`, Volume `TIMEPILOT2`)**: Dedicated game asset volume holding `PCM` audio (173 blocks with data at fixed block 7) and `MAIN4.BIN` (Slot 4 binary, 61 blocks). 241/280 blocks used (**39 blocks free**).
    - **Dynamic Drive & Capacity Auto-Detection**:
      - `disk_init()` inspects Volume Directory Key Block 2 via MLI: if `total_blocks <= 280`, dynamically engages Floppy Dual-Drive Mode (`art_unit = Drive 1, art_base_block = 128; pcm_unit = Drive 2, pcm_base_block = 7`). If `total_blocks > 280`, seamlessly defaults to single-drive HDV Mode (`art_base_block = 900; pcm_base_block = 200`), maintaining 100% backward compatibility with `TimePilot-IIvera.hdv`.
      - Normalizes unit assignments via `drive1_unit = boot_unit & 0x7F` and `drive2_unit = boot_unit | 0x80`, guaranteeing correct Drive 1 / Drive 2 mapping regardless of whether ProDOS booted or launched from D1 or D2.
      - Implemented drive ready verification: if Drive 2 is unmounted or empty when loading PCM audio, prompts the player on screen (`PLEASE INSERT DISK 2 IN DRIVE 2`) and waits for keypress before retrying.
    - **Startup Pre-Flight Probe & Disk #2 Verification (`startup.bas`)**:
      - Integrated an 89-byte 6502 MLI probe routine at `$0300`: during initial startup, `startup.bas` inspects boot volume capacity and checks Drive 2 presence. If running on 140KB floppy and Drive 2 is empty, prompts `INSERT TIME PILOT DISK #2 ON DRIVE 2. / THEN PRESS ANY KEY TO RETRY...` and loops until verified, preventing mid-boot crashes or unhandled ProDOS I/O errors.
    - **Slot 2 / Slot 4 Cross-Drive Applesoft Launcher**:
      - Enhanced `src/startup.bas` to probe VERA hardware: for Slot 2, executes `BRUN MAIN.BIN` (from Drive 1); for Slot 4, attempts `BRUN MAIN4.BIN` (succeeds on HDV) with automatic error recovery `ONERR GOTO 95` executing `BRUN MAIN4.BIN,D2` to load from Drive 2 on dual-floppy systems.
    - **Stage Art Caching & Visual `LOADING...` Banner**:
      - Cached `currentArtStage` in `upload_stage_art()`: if the current era's art is already resident in VERA Sprite RAM (e.g. entering game from title or repeating stage), redundant disk I/O is 100% skipped, enabling instant game start with zero disk reads.
      - When transitioning between eras (e.g. Stage 1 -> 2), if new art must be streamed from disk, displays a clean, centered `LOADING...` indicator at Row 14, Col 9 on the newly wiped sky, automatically erasing it once streaming completes. Players clearly see active disk loading instead of suspecting a freeze!
    - **Unified Build Pipeline**: `build.bat` automatically produces `TimePilot-IIvera.hdv`, `TimePilot-IIvera-D1.po`, and `TimePilot-IIvera-D2.po` in a single run.

71. **Official Standard 4-Voice Hardware PSG Real-Time Game Start Theme & Floppy Optimization**:
    - **Motivation & Massive Resource Savings**:
      - Promoted the 4-Voice PSG opening theme to the **official standard release** across all builds (`TimePilot-IIvera.hdv`, `TimePilot-IIvera-D1.po`, and `TimePilot-IIvera-D2.po`).
      - Replaces the 45.7 KB PCM sample with a 1,740-byte cycle-accurate PSG event table (`src/theme_psg.h`).
      - Saves **45,719 bytes (75%) of VERA VRAM Bank 0** (leaving 46,125 bytes free) and completely frees VRAM Bank 1 from audio data.
      - Saves **89 ProDOS disk blocks (44.5 KB)** across HDV and floppy images.
      - Disk 2 (`TimePilot-IIvera-D2.po`) block usage dropped from 242/280 down to **157/280 blocks (123 blocks free!)**.
    - **Timing & Playback Synchronization**:
      - Triggered right when `STAGE 1 / PLAYER 1 / A.D. 1910` appears after radar sweep.
      - Rock-solid 60Hz playback uninterrupted through 435 frames (~7.25s) while allowing player steering and propeller animation.
      - `MAIN.BIN` binary size: 32,547 bytes ($1400..$9323), leaving 733 bytes of safe headroom below the $9600 ProDOS ceiling.

72. **100% In-Memory Sprite Art & Zero Mid-Game Disk I/O**:
    - **Architecture & VRAM Allocation**:
      - Moved all remaining PCM sound effects (Coin drop, Bomb, Rocket, Boss, Timewarp, etc.) into VRAM Bank 0 (`$1000..$B70E`, 42,766 B), leaving VRAM Bank 1 (`$0000..$EFFF`, 61,440 B) 100% dedicated to sprite graphics.
      - Streamed the entire `art.blob` (59,456 bytes = 117 blocks) directly into VRAM Bank 1 at `$0000` once during boot initialization.
      - All 5 eras of player, enemies, bosses, clouds, astros, weapons, bombers, explosions, logos, and HUD icons are permanently co-resident in VRAM with zero overlap.
    - **Zero Disk Access Mid-Game**:
      - Completely removed mid-game disk streaming from `upload_stage_art()`.
      - Removed the `LOADING...` indicator and any inter-era delays or audio muting.
      - Hyperspace era transitions (Stage 0 -> 1 -> 2 -> 3 -> 4) are now instantaneous and 100% in-memory!
      - On dual floppy, Disk 1 is never accessed again after initial boot.
    - **Binary Footprint**:
      - Eliminating dynamic swapping logic reduced `MAIN.BIN` by 686 bytes to **31,861 bytes** ($1400..$9075), expanding safe headroom to **1,419 bytes** below the $9600 ProDOS limit.

73. **Authentic High Score Ranking BGM — VRAM-Resident 4-Voice PSG Loop (`AUDIO_HIGHSCORE`)**:
    - **Source Extraction**: Extracted the original Konami arcade High Score entry music from `build/name_entry.vgm` (593 AY-3-8910 register events spanning ~7.72s at 60fps) using `tools/gen_psg_highscore.mjs`.
    - **VRAM-Resident Event Table** (`HIGHSCORE_PSG_VRAM_ADDR = $D7F0`, VRAM Bank 0): 593 events × 4 bytes (delta-frame, channel+volume, freq_lo, freq_hi) = 2,372 bytes packed into `build/pcm.blob`. Zero host CPU RAM used — all playback data lives exclusively in VRAM.
    - **`highScorePsgService()` VRAM Streamer** (`src/audio.c`): Each 60Hz frame, sets VERA address with `VERA_INC_BANK0` and reads 4 bytes directly from Bank 0 into PSG channels 0–3 via `psg_write`. Delta-time scheduling via `hsWaitFrames` counter. End-marker byte (`delta=255`) triggers seamless loop.
    - **Tempo Calibration (`SPEEDUP = 1.06`)**: The raw arcade VGM plays ~6% slower than Stefan Wessels' CX16 `name_entry.pcm`. The generator script applies `SPEEDUP = 1.06` to match CX16 playback speed faithfully.
    - **Lifecycle Integration**: `audioPlaySource(AUDIO_HIGHSCORE)` starts the BGM when the initials entry screen appears; `audioStopSource(AUDIO_HIGHSCORE)` silences all 4 channels upon initials confirmation.
    - **Binary Footprint**: `MAIN.BIN` 32,191 bytes ($1400..$91BF), **1,089 bytes safe headroom** below the $9600 ProDOS ceiling.

74. **High Score Screen Jitter & Slow Music Fix — Lazy Redraw + SPEEDUP Calibration**:
    - **Root Cause of Jitter (`頓頓的`)**: `draw_hs_table()` was being called every single 60Hz frame in `state = 2`, writing ~5 rows × 3 fields × ~8 chars = 120 VERA address setups + 240 VERA data writes per frame. On the 1.02 MHz 6502 this consumed excessive CPU time in every frame, starving `audioServiceAudio()` and causing PSG event scheduling to slip frames.
    - **Lazy Redraw Fix**: Replaced unconditional `draw_hs_table()` call with a `titleClear`-gated dirty-flag check. The table is now redrawn only once when entering `state = 2` (via `check_and_start_hs_entry`). Only the blinking cursor (`draw_initials()`, ~3 chars) updates every frame, reducing VERA traffic by ≥95%.
    - **Root Cause of Slow Music**: The `SPEEDUP = 1.06` factor in `tools/gen_psg_highscore.mjs` had been accidentally removed in a prior edit. Without it, PSG events played at raw arcade VGM tempo (~6% slower than CX16 reference). Restored `SPEEDUP = 1.06` with inline comment explaining the tempo calibration rationale.
    - **Result**: High Score screen runs at rock-solid 60 FPS with responsive blinking cursor; BGM plays at authentic CX16 tempo.

75. **Arbitrary Boot Drive Support (Drive 1, Drive 2, Drive n) for HDV / ProDOS Images**:
    - **Bug & Root Cause**: When mounting `TimePilot-IIvera.hdv` in Drive 2 (e.g. Slot 7 Drive 2 `$F0`) alongside another drive in Drive 1 (e.g. `h32mb.2mg` `$70`), the screen displayed scrambled/corrupted graphics tiles. In `src/disk.c`, `disk_init()` forced `drive1_unit = boot_unit & 0x7F` and bound both `art_unit` and `pcm_unit` to `drive1_unit`. Additionally, it read volume directory block 2 from `drive1_unit` instead of the actual `boot_unit`. As a result, the game attempted to stream sprite art (block 900) and audio from Drive 1 (`h32mb.2mg`), corrupting the VERA patterns and tilemaps.
    - **Fix**:
      - `disk_init()` now reads block 2 directly from `boot_unit` (`*(volatile uint8_t *)0xBF30`) to inspect `total_blocks`.
      - In HDV mode (`total_blocks > 280`), `art_unit`, `pcm_unit`, `drive1_unit`, and `drive2_unit` are all set to `boot_unit`.
      - All asset streaming (`disk_ensure()`) is strictly pinned to `boot_unit`, allowing `TimePilot-IIvera.hdv` to boot and load seamlessly from Drive 1, Drive 2, or any slot/drive (`Drive n`).
      - In dual floppy mode (`total_blocks <= 280`), `drive1_unit = boot_unit & 0x7F` and `drive2_unit = boot_unit | 0x80` remain intact for Disk 1 / Disk 2 dual-drive pairing.
    - **Result**: Hard drive image can be placed and booted from any slot and any drive without asset loading collision.
...
- **Authentic High Score BGM** (`AUDIO_HIGHSCORE`): 4-voice VRAM-resident PSG loop extracted from arcade `name_entry.vgm`, streamed directly from VRAM Bank 0 at `$D7F0` with zero host RAM usage; `SPEEDUP=1.06` matches CX16 tempo.
- **High Score Screen Lazy Redraw**: `draw_hs_table()` now only redraws on dirty-flag, saving ≥95% VERA writes per frame and eliminating input lag / BGM slowdown.
...
1. **Pack blobs**: `mkpcm_blob.mjs` reads CX16 `.pcm` → `build/pcm.blob` (+ writes `audio_table.h`);
   `mkart.mjs` reads `src/art.h` → `build/art.blob` (+ writes `art_table.h`).
2. **Compile**: `src/main.c + audio.c + disk.c + mli.s` → `build\main.bin` (Slot 2) and `build\main4.bin` (Slot 4).
3. **Build HDV**: `build_hdv.mjs` packages 800 KB ProDOS HDV → `TimePilot-IIvera.hdv`.
4. **Build Dual 140KB Floppies**: `build_disk.mjs` packages dual 140 KB 5.25" floppy images → `TimePilot-IIvera-D1.po` and `TimePilot-IIvera-D2.po`.
