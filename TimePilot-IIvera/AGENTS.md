...
### 1. ProDOS 2.4.3 Safe Memory Map & Standard `$2000` Load Address
- **Standard ProDOS BIN Load Base (`$2000`)**: In ProDOS 2.4.3 / BASIC.SYSTEM, when executing `BRUN`, BASIC interpreter buffers and DOS parser state reside in `$0800–$0AFF`. Loading a binary at `$0800` overwrites active BASIC.SYSTEM state during file transfer, causing data corruption and screen gibberish. Therefore, standard ProDOS applications must load at **`$2000`**.
- **ProDOS Buffer Ceiling (`$9600`)**: ProDOS file buffers grow downward from `$BF00` to `$9600`. Any binary extending to `$9600` or above triggers `NO BUFFERS AVAILABLE`. The maximum safe program ceiling is `$95FF` (maximum size 30,208 bytes from `$2000`).
- **Current Verified Footprint**: `MAIN.BIN` and `MAIN4.BIN` are compiled to **30,139 bytes** (spans **`$2000–$95BA`**), providing **70 bytes of verified headroom** strictly below the `$9600` buffer boundary without `NO BUFFERS AVAILABLE` and 100% free of memory corruption! Art and audio live on HDV and are streamed via MLI `$80`.
...
| `tools/extract_cx16_sprites.py` | Python tool to extract sprites and font tiles from PNG assets into C arrays |
| `tools/mkpcm_blob.mjs` | Concatenates the CX16 PCM samples into `build/pcm.blob` + writes `audio_table.h` |
| `tools/mkart.mjs` | Reads `src/art.h`, packs sprite art into `build/art.blob` + writes `art_table.h` |
| `tools/build_hdv.mjs` | ProDOS HDV builder (MAIN.BIN + MAIN4.BIN + STARTUP + PCM/ART files) |
| `tools/build_disk.mjs` | **DEAD** — old floppy (.po) builder, superseded by `build_hdv.mjs`. Safe to delete. |
| `build.bat` | Full build: compile Slot 2 & 4 → pack blobs → build HDV |
