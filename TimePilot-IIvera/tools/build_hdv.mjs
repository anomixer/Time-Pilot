...
const pcmFileName = "pcm.blob"
const mainBinName = "main.bin"
const main4BinName = "main4.bin"
const outFileName = "TimePilot-IIvera.hdv"

if (!fs.existsSync(baseHdvPath)) throw new Error(`Base HDV not found: ${baseHdvPath}`)
if (!fs.existsSync(path.join(buildDir, mainBinName))) throw new Error(`build/${mainBinName} missing — run build.bat first`)
if (!fs.existsSync(path.join(buildDir, pcmFileName))) throw new Error(`build/${pcmFileName} missing — run tools/mkpcm_blob.mjs first`)

const disk = new Uint8Array(fs.readFileSync(baseHdvPath))
const TOTAL = disk.length / BLOCK
const mainBinRaw = new Uint8Array(fs.readFileSync(path.join(buildDir, mainBinName)))
...
const pcm = new Uint8Array(fs.readFileSync(path.join(buildDir, pcmFileName)))
...
// ---- Block allocator (skip 0..99 system reserve + audio + art ranges) ----
const used = new Set()
...
const main4Path = path.join(buildDir, main4BinName)
