...
import { getHighScorePsgEvents } from "./gen_psg_highscore.mjs"
...
const SOURCES = [
  { name: "AUDIO_COINDROP",      file: "coindrop.pcm",      bank: 0, maxSec: 0.45 },
  { name: "AUDIO_GAME_START",    file: "game_start.pcm",    bank: 0, maxSec: 7.05, tailSec: 0.0 },
  { name: "AUDIO_HIGHSCORE",     file: "highscore.pcm",     bank: 0, maxSec: 0 },
  { name: "AUDIO_NEXT_LEVEL",    file: "next_level.pcm",    bank: 0, maxSec: 1.30 },
  { name: "AUDIO_PLAYER_SHOOT",  file: "player_shoot.pcm",  bank: 0, maxSec: 0 },
  { name: "AUDIO_ROCKET_FLY",    file: "rocket_fly.pcm",    bank: 0, maxSec: 0.30, loops: 1 },
  { name: "AUDIO_BOSSL0",        file: "bossl0.pcm",        bank: 0, maxSec: 0.20, loops: 1 },
  { name: "AUDIO_BOSSL1",        file: "bossl1.pcm",        bank: 0, maxSec: 0.55, loops: 1 },
  { name: "AUDIO_BOSSL2",        file: "bossl2.pcm",        bank: 0, maxSec: 0.17, loops: 1 },
  { name: "AUDIO_BOSSL3",        file: "bossl3.pcm",        bank: 0, maxSec: 0.20, loops: 1 },
  { name: "AUDIO_WAPON_EXPLODE", file: "wapon_explode.pcm", bank: 0, maxSec: 0.35 },
  { name: "AUDIO_ENEMY_EXPLODE", file: "enemy_explode.pcm", bank: 0, maxSec: 0 },
  { name: "AUDIO_ENEMY_SHOOT",   file: "enemy_shoot.pcm",   bank: 0, maxSec: 0.20 },
  { name: "AUDIO_BOMB",          file: "bomb.pcm",          bank: 0, maxSec: 0.60 },
  { name: "AUDIO_ROCKET_LAUNCH", file: "rocket_launch.pcm", bank: 0, maxSec: 0.35 },
  { name: "AUDIO_PICKUP",        file: "pickup.pcm",        bank: 0, maxSec: 0.72, tailSec: 0.0 },
  { name: "AUDIO_EXTRA_LIFE",    file: "extra_life.pcm",    bank: 0, maxSec: 0 },
  { name: "AUDIO_WAVE_START",    file: "wave_start.pcm",    bank: 0, maxSec: 0.35 },
  { name: "AUDIO_BIG_EXPLOSION", file: "big_explosion.pcm", bank: 0, maxSec: 1.05, tailSec: 0.0 },
  { name: "AUDIO_TIMEWARP",      file: "timewarp.pcm",      bank: 0, maxSec: 1.20 },
]
...
function processPCM(raw, maxSec, tailSec) {
  ...
  // Gentle fade-out only in the final 0.05s (400 samples) to reach exact zero (prevents clicks)
  const fadeLen = Math.min(400, targetLen)
  if (fadeLen > 1) {
    for (let i = 0; i < fadeLen; i++) {
      const fade = (fadeLen - 1 - i) / (fadeLen - 1)
      out[targetLen - fadeLen + i] *= fade
    }
  }

  // Append silence tail (0x00 = signed zero in VERA 8-bit PCM) for natural decay
  const silenceSamples = Math.round((tailSec || 0) * DST_RATE)
  const totalLen = targetLen + silenceSamples

  // Pack to Uint8Array (8-bit signed PCM: 0x00 = true silence, not 0x80!)
  ...
  res.fill(0x00, targetLen) // true silence in signed 8-bit PCM is 0x00
  return res
}

function buildBlob(usePsgTheme) {
  const sources = SOURCES.map(s => {
    if (usePsgTheme && s.name === "AUDIO_GAME_START") {
      return { ...s, maxSec: 0 }
    }
    return { ...s }
  })
  ...
  // Append High Score PSG events table into Bank 0 right after PCM audio
  const hsEvents = getHighScorePsgEvents()
  const hsBuffer = Buffer.alloc(hsEvents.length * 4)
  for (let i = 0; i < hsEvents.length; i++) {
    const e = hsEvents[i]
    hsBuffer[i * 4 + 0] = e.delta
    hsBuffer[i * 4 + 1] = ((e.ch & 3) << 6) | (e.vol & 0x3F)
    hsBuffer[i * 4 + 2] = e.freq & 0xFF
    hsBuffer[i * 4 + 3] = (e.freq >> 8) & 0x3F
  }
  const hsVramAddr = VRAM_AUDIO_BASE + bank0Offset
  bank0Blob.push(hsBuffer)
  bank0Offset += hsBuffer.length
  ...
  return { sources, bank0Offset, bank1Offset, bank0Rows, bank1Rows, totalBlob, numBlocks, hsVramAddr, hsEventsCount: hsEvents.length }
}

const standard = buildBlob(true)
const test = standard

fs.writeFileSync(path.join(buildDir, "pcm.blob"), standard.totalBlob)
fs.writeFileSync(path.join(buildDir, "pcm_test.blob"), test.totalBlob)

const hex16 = (n) => "0x" + n.toString(16).toUpperCase().padStart(4, "0")

function formatSection(info) {
  const lines = [
    `#define PCM_BANK0_BYTES ${info.bank0Offset}`,
    `#define PCM_BANK1_BYTES ${info.bank1Offset}`,
    `#define PCM_TOTAL_BYTES ${info.totalBlob.length}`,
    `#define PCM_NUM_BLOCKS  ${info.numBlocks}`,
    `#define HIGHSCORE_PSG_VRAM_ADDR ${hex16(info.hsVramAddr)}`,
    `#define HIGHSCORE_PSG_EVENTS_COUNT ${info.hsEventsCount}`,
    "",
    "static const TpAudioData audioData[NUM_AUDIO_SOURCES] = {",
  ]
  for (const s of info.sources) {
    const r = (s.bank === 0) ? info.bank0Rows.get(s.name) : info.bank1Rows.get(s.name)
    lines.push(`    { ${r.bank}, ${hex16(r.start)}, ${hex16(r.length)}, ${r.loops} }, // ${r.name}`)
  }
  lines.push("};")
  return lines.join("\n")
}
...
let header = [
  ...
  "",
  formatSection(standard),
  ""
]

fs.writeFileSync(path.join(projectRoot, "src", "audio_table.h"), header.join("\n"))

console.log(`[Standard PSG] pcm.blob: ${standard.totalBlob.length} bytes (${standard.numBlocks} blocks) -> build/pcm.blob`)
console.log(`  Bank 0: ${standard.bank0Offset} bytes ($${VRAM_AUDIO_BASE.toString(16).toUpperCase()}..$${(VRAM_AUDIO_BASE + standard.bank0Offset).toString(16).toUpperCase()}, free: ${VRAM_AUDIO_LIMIT - standard.bank0Offset} bytes)`)
console.log(`  Bank 1: ${standard.bank1Offset} bytes ($${VRAM_AUDIO_BANK1_BASE.toString(16).toUpperCase()}..$${(VRAM_AUDIO_BANK1_BASE + standard.bank1Offset).toString(16).toUpperCase()}, free: ${VRAM_AUDIO_BANK1_LIMIT - standard.bank1Offset} bytes)`)
console.log(`audio_table.h written.`)
