...
* **第三關：A.D. 1970（噴射機時代）**
  * **敵機**：早期噴射戰鬥機，發射高速旋轉的導向迴力鏢（`boomerang`）。
...
* **第五關：A.D. 2001（未來太空時代）**
  ...
  * **敵機**：敏捷的脈衝幽浮（UFO），發射 8 影格色彩變換的能量雷射脈衝光球（`sbullet`）！
...
* **22 步動態超空間曲速光束**：倒數 1 秒時響起空間升頻嘯叫，背景雲朵自動隱藏確保視界純淨無阻；戰機鎖定於正中央（104, 112），亮白光束由戰機中央腰線向上下包覆展開，橫貫 28 欄全寬劇烈激盪！戰機在光柱核心高速頻閃後，光束極速凝聚塌縮為單點，戰機隨之躍遷消失！
...
* **VERA 16 通道立體聲 PSG 晶片合成（雙聲道齊奏 +6dB 爆發力）**：
  ...
  * **敵彈啾啾聲、跳傘員救援三音琶音、關卡凱旋和弦與超時空躍遷升頻**：多聲道硬體獨立合成，毫無延遲，擊落再多敵機也絕不卡音！
* **雙 Bank VRAM 常駐 11 首街機 PCM 音訊（100% 零磁碟運行串流）**：
  * **Bank 0 (`$1000..$FE58`)**：開場主題曲 (`AUDIO_GAME_START`, 8.05 秒完整尾韻) 與重低音大爆炸 (`AUDIO_BIG_EXPLOSION`, 1.38 秒正宗震撼爆炸)。
  * **Bank 1 (`$2800..$7616`)**：投幣音效 (`AUDIO_COINDROP`)、四機突襲警報 (`AUDIO_WAVE_START`)、1940 轟炸機空投航彈呼嘯 (`AUDIO_BOMB`)、1970/1982 飛彈點火與巡航 (`AUDIO_ROCKET_LAUNCH` / `AUDIO_ROCKET_FLY`)、四大關卡 Boss 巨型母艦專屬警報 (`AUDIO_BOSSL0 ~ 3`)。
* **智慧動態音量平衡**：
  * Boss 警報與巡航蜂鳴適度柔化（Vol 7/15），營造緊張大氣氛圍；激烈的雷射開火與近身爆炸依然清晰響亮，達到完美的聽覺平衡！
