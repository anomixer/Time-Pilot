...
1. **One-Time Boot Streaming (VERA 128KB VRAM as High-Speed SSD)**:
   * During boot, ProDOS Direct Block MLI (`$80`) streams **82,730 bytes (162 disk blocks) of 12 authentic arcade PCM samples** (game start theme, heavy explosions, time warp whoosh, bombs, sirens, and missiles) and **56,640 bytes (111 disk blocks) of all-era sprite artwork** directly into VERA's 128 KB dual-bank VRAM (Bank 0 and Bank 1).
...
| **Audio Hardware** | VERA PCM FIFO + YM2151 FM | Ensoniq 5503 DOC (32-oscillator wavetable) | **VERA 16-Channel Stereo PSG (+6dB unison) + 12 Resident PCM Samples** |
...
| **Sound System** | Single PCM queue | Ensoniq DOC wavetable | **Dual Hybrid Sound Engine**: 16-channel PSG (+6dB laser, explosion rumble) + 12 PCM samples |
...
| Input / Key | Action |
| :--- | :--- |
| `A` or `←` (Left Arrow) | Rotate counter-clockwise (11.25° per step, 32 headings total) |
| `D` or `→` (Right Arrow) | Rotate clockwise (11.25° per step, 32 headings total) |
| `W` / `A` / `S` / `D` | Quick-snap to 8 cardinal/diagonal directions |
...
2. **Dual-Bank VRAM Resident 12 Arcade PCM Samples**:
   ...
   * **Bank 1 (`$1200..$6F64`, 23.9 KB)**: `AUDIO_COINDROP`, `AUDIO_ROCKET_LAUNCH`, `AUDIO_ROCKET_FLY`, `AUDIO_WAVE_START`, `AUDIO_BOSSL0..3`, and `AUDIO_TIMEWARP` (1.20s whoosh).
...
1. **開機一次載入，VRAM 充當超高速板載 SSD**：
   * 開機引導階段透過 ProDOS MLI 直讀，一口氣將 **82,730 位元組（162 個磁區）的 12 首街機 PCM 取樣**（包含開場音樂、重低音大爆炸、時空躍遷 Time Warp 穿梭音等）與 **56,640 位元組（111 個磁區）的全時代精靈圖庫**，直接寫入 VERA 擴充卡的 128 KB 獨立雙 Bank 記憶體（Bank 0 與 Bank 1）。
...
| **音效硬體架構** | VERA PCM FIFO (單聲道) + YM2151 FM | Ensoniq 5503 DOC (32 振盪器波表晶片) | **VERA 16 通道立體聲 PSG (雙聲道齊奏 +6dB) + 雙 Bank VRAM 常駐 12 首 PCM 音效** |
...
| **多音軌音效架構** | 單軌 PCM 優先權互斥佇列 | Ensoniq DOC 專屬多聲道波表合成 | **極限複合雙音效引擎**：<br>1. **PSG 雙聲道疊加齊奏 (+6dB)**：雷射、爆破、敵彈、凱旋和弦多聲道並行！<br>2. **雙 Bank 常駐 12 首 PCM**：開場曲、大爆炸、投幣、炸彈、飛彈、時空躍遷與四大 Boss 警報無縫串流！ |
...
欲了解本專案的底層硬體暫存器映射、ProDOS 安全記憶體跨度、VRAM Bank 0/1 配置、零乘法八分圓幾何解角器與完整 58 項工程里程碑，請參閱詳細工程指南：  
