...
#ifdef USE_PSG_THEME
#include "theme_psg.h"
static uint16_t themeEventIdx = 0;
static uint8_t  themeWaitFrames = 0;
static uint8_t  themeActive = 0;

static void themePsgStart(void) {
    themeActive = 1;
    themeEventIdx = 0;
    themeWaitFrames = themePsgEvents[0].delta;
    for (uint8_t c = 0; c < 4; c++) {
        psg_silence(CH_THEME_BASE + c);
    }
}

static void themePsgStop(void) {
    if (themeActive) {
        themeActive = 0;
        for (uint8_t c = 0; c < 4; c++) {
            psg_silence(CH_THEME_BASE + c);
...
static void themePsgService(void) {
    if (!themeActive) return;

    if (themeWaitFrames > 0) {
        themeWaitFrames--;
    }

    while (themeWaitFrames == 0 && themeEventIdx < PSG_THEME_EVENTS_COUNT) {
        const PsgThemeEvent *e = &themePsgEvents[themeEventIdx++];
        if (e->delta == 255) {
            themePsgStop();
            activePlaying = -1;
            break;
        ...
        uint8_t ch = (e->ch_vol >> 6) & 3;
        uint8_t vol = e->ch_vol & 0x3F;
        if (vol > 0 && e->freq > 0) {
            psg_write(CH_THEME_BASE + ch, e->freq, 0xC0 | vol, 0x3F);
        } else {
            psg_silence(CH_THEME_BASE + ch);
        }

        if (themeEventIdx < PSG_THEME_EVENTS_COUNT) {
            uint8_t nextDelta = themePsgEvents[themeEventIdx].delta;
            if (nextDelta > 0) {
                themeWaitFrames = nextDelta;
                break;
            }
        }
    }
...
/* ---------------- High Score Ranking PSG Theme (Bank 0 VRAM) ---------------- */
static uint16_t hsPsgVramAddr = 0;
static uint8_t  hsWaitFrames = 0;
static uint8_t  hsActive = 0;

static void highScorePsgStart(void) {
    hsActive = 1;
    hsPsgVramAddr = HIGHSCORE_PSG_VRAM_ADDR;
    vera_set_addr(0x00, HIGHSCORE_PSG_VRAM_ADDR);
    hsWaitFrames = VERA.data0;
    for (uint8_t c = 0; c < 4; c++) {
        psg_silence(CH_THEME_BASE + c);
    }
}

static void highScorePsgStop(void) {
    if (hsActive) {
        hsActive = 0;
        for (uint8_t c = 0; c < 4; c++) {
            psg_silence(CH_THEME_BASE + c);
...
static void highScorePsgService(void) {
    if (!hsActive) return;
    ...
    while (hsWaitFrames == 0 && hsActive) {
        vera_set_addr(VERA_INC_BANK0, hsPsgVramAddr);
        uint8_t delta = VERA.data0;
        uint8_t ch_vol = VERA.data0;
        uint8_t freq_lo = VERA.data0;
        uint8_t freq_hi = VERA.data0;
        hsPsgVramAddr += 4;

        uint8_t ch = (ch_vol >> 6) & 3;
        uint8_t vol = ch_vol & 0x3F;
        uint16_t freq = (uint16_t)freq_lo | ((uint16_t)freq_hi << 8);

        if (vol > 0 && freq > 0) {
            psg_write(CH_THEME_BASE + ch, freq, 0xC0 | vol, 0x3F);
        } else {
            psg_silence(CH_THEME_BASE + ch);
        }

        /* Check next event delta without advancing */
        vera_set_addr(0x00, hsPsgVramAddr);
        uint8_t nextDelta = VERA.data0;
        if (nextDelta == 255) {
            /* End of track: loop high score theme */
            hsPsgVramAddr = HIGHSCORE_PSG_VRAM_ADDR;
            vera_set_addr(0x00, HIGHSCORE_PSG_VRAM_ADDR);
            hsWaitFrames = VERA.data0;
            break;
...
#ifdef USE_PSG_THEME
    if (source == AUDIO_GAME_START) return themeActive;
#endif
    if (source == AUDIO_HIGHSCORE) return hsActive;
...
#ifdef USE_PSG_THEME
    if (source == AUDIO_GAME_START) {
        themePsgStop();
        activePlaying = -1;
        return;
    }
#endif
    if (source == AUDIO_HIGHSCORE) {
        highScorePsgStop();
        activePlaying = -1;
        return;
    }
...
void audioPlaySource(int8_t source) {
    ...
    case AUDIO_HIGHSCORE:
        highScorePsgStart();
        break;

    case AUDIO_NEXT_LEVEL:
        /* Authentic Arcade / CX16 Stage Clear Victory Fanfare PCM */
        pcm_play(audioData[AUDIO_NEXT_LEVEL].bank, audioData[AUDIO_NEXT_LEVEL].start, audioData[AUDIO_NEXT_LEVEL].length, audioData[AUDIO_NEXT_LEVEL].loops, 12);
        break;
    ...
    case AUDIO_PICKUP:
        /* Authentic Arcade Parachute Pilot rescue pickup PCM (full multi-tone melody, balanced volume) */
        psg_silence(CH_PICKUP);
        pcm_play(audioData[AUDIO_PICKUP].bank, audioData[AUDIO_PICKUP].start, audioData[AUDIO_PICKUP].length, audioData[AUDIO_PICKUP].loops, 8);
        break;
    ...
    case AUDIO_WAVE_START:
        /* Authentic 4-plane formation attack siren PCM (balanced volume) */
        pcm_play(audioData[AUDIO_WAVE_START].bank, audioData[AUDIO_WAVE_START].start, audioData[AUDIO_WAVE_START].length, audioData[AUDIO_WAVE_START].loops, 8);
        break;
...
void audioServiceAudio(void) {
    ...
    if (sfxTimer[CH_EXPL]) {
        ...
        if (sfxTimer[CH_EXPL] == 0) {
            ...
            if (sfxTimer[CH_EXPL] < 16) {
                if (sfxVol[CH_EXPL] >= 4) sfxVol[CH_EXPL] -= 4;
                else sfxVol[CH_EXPL] = 0;
            }
